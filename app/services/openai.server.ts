...
Please provide the response in the following JSON format:
{
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an expert SEO content writer specializing in e-commerce blog posts. Always respond with valid JSON.",
      },
      {
        role: "user",
        content: prompt,
...
Respond in JSON format:
{
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an SEO expert. Generate compelling meta tags that improve click-through rates. Always respond with valid JSON.",
      },
      {
        role: "user",
        content: prompt,
...
export async function generateAltText(
  ...
  const { imageUrl, productTitle, productDescription, context } = params;
...
Respond with just the alt text, no quotes or additional formatting.`;
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an accessibility and SEO expert. Generate descriptive, keyword-rich alt text for images.",
      },
      {
        role: "user",
        content: prompt,
...
Respond in JSON format:
{
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an SEO keyword research expert. Provide realistic keyword suggestions based on common search patterns. Always respond with valid JSON.",
      },
      {
        role: "user",
        content: prompt,
...
Respond in JSON format:
{
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an SEO content analyst. Provide specific, actionable suggestions for improving content SEO. Always respond with valid JSON.",
      },
      {
        role: "user",
        content: prompt,
